    ...
    ms.system.run(() => {
        if (chatMessage.sender.hasTag(adminTag)) {
            if (!args[1]) {
                ...
                if (args[2] == "global") {
                    ms.system.run(() => {
                        ms.world.scoreboard.removeObjective('dirtletsEssentials:save:mail');
                        chatMessage.sender.dimension.runCommand(`
                            tellraw "${chatMessage.sender.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6Sucessfully cleared the mail system.§r"}]}
                        `);
                        chatMessage.sender.dimension.runCommand(`
                            tellraw @a[tag="${adminTag}",tag=!"${messageHideTag}",name=!"${chatMessage.sender.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6Sucessfully cleared the mail system.§r"}]}
                        `);
                    });
                }
                else {
                    ms.system.run(() => {
                        let array = ms.world.scoreboard.getObjective('dirtletsEssentials:save:mail').getParticipants()[0].displayName.split("#@##@&*£$strsplit#@##@&*£$");
                        ms.world.scoreboard.removeObjective('dirtletsEssentials:save:mail');
