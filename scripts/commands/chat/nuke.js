        ...
        if (chatMessage.sender.hasTag(adminTag)) {
            ...
            if (ms.world.scoreboard.getObjective('associatedRealmsEssentials:command:nuke').getParticipants()[0].displayName == chatMessage.sender.name) {
                chatMessage.sender.dimension.runCommand(`
                    tellraw "${chatMessage.sender.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§eNuke has been §cdisabled.§r"}]}
                `);
                chatMessage.sender.dimension.runCommand(`
                    tellraw @a[tag="${adminTag}",tag=!"${messageHideTag}",name=!"${chatMessage.sender.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§e${chatMessage.sender.name} > Nuke has been §cdisabled.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                `);
            ...
            // Nuke On
            chatMessage.sender.dimension.runCommand(`
                tellraw "${chatMessage.sender.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§eNuke has been §aenabled.§r"}]}
            `);
            chatMessage.sender.dimension.runCommand(`
                tellraw @a[tag="${adminTag}",tag=!"${messageHideTag}",name=!"${chatMessage.sender.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§e${chatMessage.sender.name} > Nuke has been §aenabled.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
            `);
            ms.world.scoreboard.removeObjective('associatedRealmsEssentials:command:nuke');
            ms.world.scoreboard.addObjective('associatedRealmsEssentials:command:nuke', 'associatedRealmsEssentials:command:nuke');
