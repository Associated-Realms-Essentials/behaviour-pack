...
function runCommandChatAdminUi(chatMessage, adminTag, messageHideTag, prefix) {
    ms.system.run(() => {
        ...
        ms.system.run(() => {
            if (chatMessage.sender.hasTag(adminTag)) {
                sendCommandMessageForInitiatorAndAdmins("§e", "Close Chat to view the Admin UI...");
                openAdminFormMain(chatMessage, adminTag, messageHideTag, prefix);
                return;
            }
            sendCommandMessageForInitiatorAndAdmins("§c", "Insufficient Permissions.");
        });
    });
}
export default runCommandChatAdminUi;
