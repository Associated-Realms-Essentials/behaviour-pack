    ...
    ms.system.run(() => {
        if (chatMessage.sender.hasTag(adminTag)) {
            ...
            if (ms.world.scoreboard.getObjective('dirtletsEssentials:joinMessages:text').getParticipants()[0].displayName == "") {
                chatMessage.sender.dimension.runCommand(`
                    tellraw @a {"rawtext":[{"text":"\n§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§lEveryone Welcome ${chatMessage.sender.name}!§r"}]}
                `);
                return;
            }
