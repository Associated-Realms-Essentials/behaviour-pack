...
function runCommandChatListBuild(chatMessage, adminTag, messageHideTag, prefix, args) {
    ms.system.run(() => {
        if (chatMessage.sender.hasTag(adminTag)) {
            ...
            ms.world.scoreboard.getObjective('dirtletsEssentials:save:buildAll').getParticipants().forEach(buildGlobalState => {
            ...
            ms.world.scoreboard.getObjective('dirtletsEssentials:save:buildPeople').getParticipants().forEach(buildDbItem => {
                chatMessage.sender.dimension.runCommand(`
