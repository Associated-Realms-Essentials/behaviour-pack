...
function runEventBeforeChatSend(chatMessage, adminTag, prefix, playerArray, messageHideTag, blockedTag) {
    ...
    ms.system.run(() => {
        ...
        ms.world.scoreboard.getObjective('dirtletsEssentials:save:chat').getParticipants().forEach(msgs => {
            associatedIndex += 1;
        });
        chatMessage.sender.dimension.runCommand(`scoreboard players set "${sanitizeStringForMessage(chatMessage.message)}#@##@&*£$strsplit#@##@&*£$${sanitizeStringForUsername(chatMessage.sender.name)}#@##@&*£$strsplit#@##@&*£$${associatedIndex + 1}" "dirtletsEssentials:save:chat" 0`);
    });
    ...
    // return}
    let isMuted = false;
    ms.world.scoreboard.getObjective('dirtletsEssentials:save:mutedPeople').getParticipants().forEach(mutedDbItem => {
        ...
        if (chatMessage.message.startsWith(prefix + "mute")) {
            ...
            ms.system.run(() => {
                chatMessage.sender.dimension.runCommand(`
                    tellraw "${chatMessage.sender.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6You are muted, you cannot chat at this point.§r"}]}
                `);
                chatMessage.sender.dimension.runCommand(`
                    tellraw @a[tag="${adminTag}",tag=!"${messageHideTag}",name=!"${chatMessage.sender.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6${chatMessage.sender.name} > You are muted, you cannot chat at this point.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                `);
            });
        }
    }
            ...
            if (!chatMessage.message.startsWith(prefix)) {
                ...
                ms.system.run(() => {
                    if (chatMessage.sender.hasTag(adminTag)) {
                        ...
                        `);
                        chatMessage.sender.dimension.runCommand(`
                            tellraw @a[tag="${adminTag}",tag=!"${messageHideTag}",name=!"${chatMessage.sender.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§4${chatMessage.sender.name} > Invalid Command.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                        `);
                        return;
                    }
                    chatMessage.sender.dimension.runCommand(`
