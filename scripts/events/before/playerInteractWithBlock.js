                ...
                form.show(block.player).then(formRes => {
                    ...
                    if (formRes.formValues[0] == "") {
                        block.player.dimension.runCommand(`
                            tellraw @a[tag="${adminTag}", tag=!"${messageHideTag}", name=!"${block.player.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6${block.player.name} > §cThe floating text has not been created because you havent given any text.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                            `);
                        block.player.dimension.runCommand(`
                            tellraw "${block.player.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§a§cThe floating text has not been created because you havent given any text.§r"}]}
                            `);
                        return;
                    }
                    if (formRes.formValues[1] == "") {
                        block.player.dimension.runCommand(`
                            tellraw @a[tag="${adminTag}", tag=!"${messageHideTag}", name=!"${block.player.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6${block.player.name} > §cThe floating text has not been created because you havent specifyed coordinates.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                            `);
                        block.player.dimension.runCommand(`
                            tellraw "${block.player.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§a§cThe floating text has not been created because you havent specifyed coordinates.§r"}]}
                            `);
                        return;
                    }
                    let isNumber = false;
                    ...
                    if (chords[2] == "") {
                        block.player.dimension.runCommand(`
                                tellraw @a[tag="${adminTag}", tag=!"${messageHideTag}", name=!"${block.player.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6${block.player.name} > §cNot valid coordinates.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                            `);
                        block.player.dimension.runCommand(`
                                tellraw "${block.player.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§a§cNot valid coordinates.§r"}]}
                            `);
                        return;
                    }
                    ...
                    if (isNumber) {
                        ...
                        try {
                            ...
                            block.player.dimension.runCommand(`
                                    tellraw @a[tag="${adminTag}", tag=!"${messageHideTag}", name=!"${block.player.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6${block.player.name} > §c${err} ${err.stack}\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                                `);
                            block.player.dimension.runCommand(`
                                    tellraw "${block.player.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§a§c${err} ${err.stack}§r"}]}
                                `);
                        }
                    }
                });
        ...
        if (block.block.getRedstonePower() == 0) {
            if (block.player.getComponent('inventory').container?.getSlot(block.player.selectedSlotIndex).getItem().typeId.includes("music_disc_")) {
                ...
                if (block.player.hasTag(adminTag)) {
                    ms.system.run(() => {
                        block.player.dimension.runCommand(`
                                    tellraw @a[tag="${adminTag}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§aEnjoy the music §2${block.player.name}!§r"}]}
                                `);
                    });
                    return;
                }
                ms.system.run(() => {
                    block.player.dimension.runCommand(`
                            ...
                            `);
                    block.player.dimension.runCommand(`
                                tellraw "${block.player.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§aEnjoy the music §2${block.player.name}!§r"}]}
                            `);
                });
            }
            return;
            ...
            if (Math.floor(block.block.location.x) == parseInt(array[0])) {
                if (Math.floor(block.block.location.y) == parseInt(array[1])) {
                    if (Math.floor(block.block.location.z) == parseInt(array[2])) {
                        ...
                        ms.system.run(() => {
                            block.player.dimension.runCommand(`
                                    tellraw @a[tag="${adminTag}", tag=!"${messageHideTag}", name=!"${block.player.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6${block.player.name} > §eThis chest has been §clocked§e by a admin.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                                `);
                            block.player.dimension.runCommand(`
                                    tellraw "${block.player.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§a§eThis chest has been §clocked§e by a admin.§r"}]}
                                `);
                        });
                    }
                }
            ...
            if (block.itemStack.nameTag == "arefillwand") {
                ...
                ms.system.run(() => {
                    ...
                    if (playerStillExists == false) {
                        ...
                        block.player.dimension.runCommand(`scoreboard players set "" "dirtletsEssentials:fillWand:block" 0`);
                        ...
                        block.player.dimension.runCommand(`scoreboard players set "" "dirtletsEssentials:fillWand:coordsSet1" 0`);
                        ...
                        block.player.dimension.runCommand(`scoreboard players set "" "dirtletsEssentials:fillWand:coordsSet2" 0`);
                        ...
                        block.player.dimension.runCommand(`scoreboard players set "" "dirtletsEssentials:fillWand:player" 0`);
                    }
                    if (ms.world.scoreboard.getObjective('dirtletsEssentials:fillWand:coordsSet1').getParticipants()[0].displayName == "") {
                        if (ms.world.scoreboard.getObjective('dirtletsEssentials:fillWand:player').getParticipants()[0].displayName == "") {
                            ms.world.scoreboard.removeObjective('dirtletsEssentials:fillWand:player');
                            ms.world.scoreboard.addObjective('dirtletsEssentials:fillWand:player', 'dirtletsEssentials:fillWand:player');
                            block.player.dimension.runCommand(`scoreboard players set "${block.player.name}" "dirtletsEssentials:fillWand:player" 0`);
                        }
                        ms.world.scoreboard.removeObjective('dirtletsEssentials:fillWand:coordsSet1');
                        ms.world.scoreboard.addObjective('dirtletsEssentials:fillWand:coordsSet1', 'dirtletsEssentials:fillWand:coordsSet1');
                        block.player.dimension.runCommand(`scoreboard players set "${block.block.location.x} ${block.block.location.y} ${block.block.location.z}" "dirtletsEssentials:fillWand:coordsSet1" 0`);
                        ms.world.scoreboard.removeObjective('dirtletsEssentials:fillWand:block');
                        ms.world.scoreboard.addObjective('dirtletsEssentials:fillWand:block', 'dirtletsEssentials:fillWand:block');
                        block.player.dimension.runCommand(`scoreboard players set "${block.block.typeId}" "dirtletsEssentials:fillWand:block" 0`);
                        block.player.dimension.runCommand(`
                                        tellraw @a[tag="${adminTag}", tag=!"${messageHideTag}", name=!"${block.player.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6${block.player.name} > §eSucessfully set coords '${ms.world.scoreboard.getObjective('dirtletsEssentials:fillWand:coordsSet1').getParticipants()[0].displayName}' for set 1.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                                    `);
                        block.player.dimension.runCommand(`
                                        tellraw "${block.player.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§a§eSucessfully set coords '${ms.world.scoreboard.getObjective('dirtletsEssentials:fillWand:coordsSet1').getParticipants()[0].displayName}' for set 1.§r"}]}
                                    `);
                    }
                    else {
                        if (ms.world.scoreboard.getObjective('dirtletsEssentials:fillWand:coordsSet2').getParticipants()[0].displayName == "") {
                            if (block.player.name !== ms.world.scoreboard.getObjective('dirtletsEssentials:fillWand:player').getParticipants()[0].displayName) {
                                block.player.dimension.runCommand(`
                                        tellraw @a[tag="${adminTag}", tag=!"${messageHideTag}", name=!"${block.player.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6${block.player.name} > §cYou must let ${ms.world.scoreboard.getObjective('dirtletsEssentials:fillWand:player').getParticipants()[0].displayName} finish up with the fill wand in order for you to start using it.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                                    `);
                                block.player.dimension.runCommand(`
                            ...
                            ms.world.scoreboard.removeObjective('dirtletsEssentials:fillWand:coordsSet2');
                            ms.world.scoreboard.addObjective('dirtletsEssentials:fillWand:coordsSet2', 'dirtletsEssentials:fillWand:coordsSet2');
                            block.player.dimension.runCommand(`scoreboard players set "${block.block.location.x} ${block.block.location.y} ${block.block.location.z}" "dirtletsEssentials:fillWand:coordsSet2" 0`);
                            ...
                            if (result.successCount == 0) {
                                block.player.dimension.runCommand(`
                                                tellraw @a[tag="${adminTag}", tag=!"${messageHideTag}", name=!"${block.player.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6${block.player.name} > §cCould not fill the selected area, the area you selected was too big to fill or the fill block was identical to the selected blocks.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                                            `);
                                block.player.dimension.runCommand(`
                                                tellraw "${block.player.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§cCould not fill the selected area, the area you selected was too big to fill or the fill block was identical to the selected blocks.§r"}]}
                                            `);
                            }
                            else {
                                block.player.dimension.runCommand(`
