            ...
            if (passed == false) {
                ...
                form.show(item.source).then(formRes => {
                    ...
                    if (formRes.formValues[0] == "") {
                        ...
                        tellraw @a[tag="${adminTag}", tag=!"${messageHideTag}", name=!"${item.source.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6${item.source.name} > §cThe floating text has not been created because you havent given any text.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                        `);
                        item.source.dimension.runCommand(`
                        tellraw "${item.source.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§a§cThe floating text has not been created because you havent given any text.§r"}]}
                        `);
                        return;
                    }
                    if (formRes.formValues[1] == "") {
                        item.source.dimension.runCommand(`
                        tellraw @a[tag="${adminTag}", tag=!"${messageHideTag}", name=!"${item.source.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6${item.source.name} > §cThe floating text has not been created because you havent specifyed coordinates.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                        `);
                        item.source.dimension.runCommand(`
                        tellraw "${item.source.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§a§cThe floating text has not been created because you havent specifyed coordinates.§r"}]}
                        `);
                        return;
                    }
                    let isNumber = false;
                    ...
                    if (chords[2] == "") {
                        item.source.dimension.runCommand(`
                            tellraw @a[tag="${adminTag}", tag=!"${messageHideTag}", name=!"${item.source.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6${item.source.name} > §cNot valid coordinates.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                        `);
                        item.source.dimension.runCommand(`
                            tellraw "${item.source.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§a§cNot valid coordinates.§r"}]}
                        `);
                        return;
                    }
                    ...
                    if (isNumber) {
                        ...
                        try {
                            ...
                            item.source.dimension.runCommand(`
                                tellraw @a[tag="${adminTag}", tag=!"${messageHideTag}", name=!"${item.source.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6${item.source.name} > §c${err} ${err.stack}\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                            `);
                            item.source.dimension.runCommand(`
                                tellraw "${item.source.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§a§c${err} ${err.stack}§r"}]}
                            `);
                        }
                    }
                });
