...
function runEventBeforePlayerBreakBlock(block, adminTag, messageHideTag) {
    ...
    ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:buildPeople').getParticipants().forEach(user => {
                ...
                if (beforeBlock.itemStack.nameTag == "arefillwand") {
                    ...
                    ms.system.run(() => {
                        ...
                        if (playerStillExists == false) {
                            ...
                            beforeBlock.player.dimension.runCommand(`scoreboard players set "" "associatedRealmsEssentials:fillWand:block" 0`);
                            ...
                            beforeBlock.player.dimension.runCommand(`scoreboard players set "" "associatedRealmsEssentials:fillWand:coordsSet1" 0`);
                            ...
                            beforeBlock.player.dimension.runCommand(`scoreboard players set "" "associatedRealmsEssentials:fillWand:coordsSet2" 0`);
                            ...
                            beforeBlock.player.dimension.runCommand(`scoreboard players set "" "associatedRealmsEssentials:fillWand:player" 0`);
                        }
                        if (ms.world.scoreboard.getObjective('associatedRealmsEssentials:fillWand:coordsSet1').getParticipants()[0].displayName == "") {
                            if (ms.world.scoreboard.getObjective('associatedRealmsEssentials:fillWand:player').getParticipants()[0].displayName == "") {
                                ms.world.scoreboard.removeObjective('associatedRealmsEssentials:fillWand:player');
                                ms.world.scoreboard.addObjective('associatedRealmsEssentials:fillWand:player', 'associatedRealmsEssentials:fillWand:player');
                                beforeBlock.player.dimension.runCommand(`scoreboard players set "${beforeBlock.player.name}" "associatedRealmsEssentials:fillWand:player" 0`);
                            }
                            ms.world.scoreboard.removeObjective('associatedRealmsEssentials:fillWand:coordsSet1');
                            ms.world.scoreboard.addObjective('associatedRealmsEssentials:fillWand:coordsSet1', 'associatedRealmsEssentials:fillWand:coordsSet1');
                            beforeBlock.player.dimension.runCommand(`scoreboard players set "${beforeBlock.block.location.x} ${beforeBlock.block.location.y} ${beforeBlock.block.location.z}" "associatedRealmsEssentials:fillWand:coordsSet1" 0`);
                            ms.world.scoreboard.removeObjective('associatedRealmsEssentials:fillWand:block');
                            ms.world.scoreboard.addObjective('associatedRealmsEssentials:fillWand:block', 'associatedRealmsEssentials:fillWand:block');
                            beforeBlock.player.dimension.runCommand(`scoreboard players set "minecraft:air" "associatedRealmsEssentials:fillWand:block" 0`);
                            // xyz1 Selection
                            beforeBlock.player.dimension.runCommand(`
                                            tellraw @a[tag="${adminTag}", tag=!"${messageHideTag}", name=!"${beforeBlock.player.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6${beforeBlock.player.name} > §eSucessfully set coords '${ms.world.scoreboard.getObjective('associatedRealmsEssentials:fillWand:coordsSet1').getParticipants()[0].displayName}' for set 1.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                                        `);
                            beforeBlock.player.dimension.runCommand(`
                            ...
                            if (ms.world.scoreboard.getObjective('associatedRealmsEssentials:fillWand:coordsSet2').getParticipants()[0].displayName == "") {
                                ms.system.run(() => {
                                    if (beforeBlock.player.name !== ms.world.scoreboard.getObjective('associatedRealmsEssentials:fillWand:player').getParticipants()[0].displayName) {
                                        beforeBlock.player.dimension.runCommand(`
                                            tellraw @a[tag="${adminTag}", tag=!"${messageHideTag}", name=!"${beforeBlock.player.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6${beforeBlock.player.name} > §cYou must let ${ms.world.scoreboard.getObjective('associatedRealmsEssentials:fillWand:player').getParticipants()[0].displayName} finish up with the fill wand in order for you to start using it.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                                        `);
                                        beforeBlock.player.dimension.runCommand(`
                                    ...
                                    ms.world.scoreboard.removeObjective('associatedRealmsEssentials:fillWand:coordsSet2');
                                    ms.world.scoreboard.addObjective('associatedRealmsEssentials:fillWand:coordsSet2', 'associatedRealmsEssentials:fillWand:coordsSet2');
                                    beforeBlock.player.dimension.runCommand(`scoreboard players set "${beforeBlock.block.location.x} ${beforeBlock.block.location.y} ${beforeBlock.block.location.z}" "associatedRealmsEssentials:fillWand:coordsSet2" 0`);
                                });
                                ms.system.run(() => {
                                    ...
                                    if (result.successCount == 0) {
                                        ...
                                        beforeBlock.player.dimension.runCommand(`
                                                    tellraw @a[tag="${adminTag}", tag=!"${messageHideTag}", name=!"${beforeBlock.player.name}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§6${beforeBlock.player.name} > §cCould not fill the selected area, the area you selected was too big to fill or the fill block was identical to the selected blocks.\n§r(give yourself the '${messageHideTag}' tag to silence these messages.)§r"}]}
                                                `);
                                        beforeBlock.player.dimension.runCommand(`
                                                    tellraw "${beforeBlock.player.name}" {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§cCould not fill the selected area, the area you selected was too big to fill or the fill block was identical to the selected blocks.§r"}]}
                                                `);
                                        beforeBlock.player.runCommand(`
