    ...
    ms.system.run(() => {
        let res = (block) => {
            block.player.dimension.runCommand(`
                tellraw @a[tag="${adminTag}", tag=!"${alertHideTag}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('dirtletsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§e${block.block.typeId.split(":")[1]} has been placed by ${block.player.name} at §c${block.block.location.x} ${block.block.location.y} ${block.block.location.z}§e.§r"}]}
            `);
            block.player.dimension.runCommand(`scoreboard players set "${block.block.typeId}#@##@&*£$strsplit#@##@&*£$${block.player.name}#@##@&*£$strsplit#@##@&*£$${block.block.location.x}#@##@&*£$strsplit#@##@&*£$${block.block.location.y}#@##@&*£$strsplit#@##@&*£$${block.block.location.z}#@##@&*£$strsplit#@##@&*£$place" "dirtletsEssentials:save:blocklogs" 0`);
        };
