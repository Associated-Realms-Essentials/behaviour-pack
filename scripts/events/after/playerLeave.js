...
function runEventAfterPlayerLeave(player, developers) {
    ms.system.run(() => {
        ...
        if (ms?.world?.getDimension('overworld')) {
            ms?.world?.getDimension('overworld')?.runCommand(`scoreboard players set "${player.playerName}#@##@&*£$strsplit#@##@&*£$${isoBasicFormat}#@##@&*£$strsplit#@##@&*£$leave" "associatedRealmsEssentials:save:accesslogs" 0`);
        }
    });
}
