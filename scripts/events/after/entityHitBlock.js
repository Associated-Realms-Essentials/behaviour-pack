...
function runEventAfterEntityHitBlock(block, adminTag, messageHideTag) {
    ...
    //                                         tellraw @a[tag="${adminTag}"] {"rawtext":[{"text":"§l§f§2${ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName}§r§l§a >> §r§l§e${block.hitBlock.typeId.split(":")[1]} has been broken by ${plr.name} at §c${block.hitBlock.location.x} ${block.hitBlock.location.y} ${block.hitBlock.location.z}§e.§r"}]}
    //                                     `)
    //                                     block.damagingEntity.dimension.runCommand(`scoreboard players set "${block.hitBlock.typeId}#@##@&*£$strsplit#@##@&*£$${plr.name}#@##@&*£$strsplit#@##@&*£$${block.hitBlock.location.x}#@##@&*£$strsplit#@##@&*£$${block.hitBlock.location.y}#@##@&*£$strsplit#@##@&*£$${block.hitBlock.location.z}#@##@&*£$strsplit#@##@&*£$break" "associatedRealmsEssentials:save:blocklogs" 0`)
    //                                 });
    //                             };
