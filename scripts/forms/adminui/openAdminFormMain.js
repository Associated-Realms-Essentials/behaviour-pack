...
function openAdminFormMain(chatMessage, adminTag, messageHideTag, prefix) {
    ms.system.run(() => {
        ...
        var packSettingsRenameAssociationForm = new msui.ModalFormData()
            ...
            .textField('Rename Association', '(Required)', { defaultValue: ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:association').getParticipants()[0].displayName });
        ...
        var configureFeaturesJoinMessagesForm = new msui.ModalFormData()
            .title('Join Messages')
            .toggle('Enabled', { defaultValue: JSON.parse(ms.world.scoreboard.getObjective('associatedRealmsEssentials:joinMessages:enabled').getParticipants()[0].displayName) })
            .textField('Enter Message', '(Optional)', { defaultValue: ms.world.scoreboard.getObjective('associatedRealmsEssentials:joinMessages:text').getParticipants()[0].displayName });
        var configureFeaturesJukeboxPlusForm = new msui.ModalFormData()
            .title('Jukebox+')
            .toggle('Enabled', { defaultValue: JSON.parse(ms.world.scoreboard.getObjective('associatedRealmsEssentials:jukeboxPlus:enabled').getParticipants()[0].displayName) });
        var configureFeaturesBottomBedrockForm = new msui.ModalFormData()
            .title('Bottom Bedrock')
            .toggle('Overworld', { defaultValue: JSON.parse(ms.world.scoreboard.getObjective('associatedRealmsEssentials:bottomBedrock:overworld').getParticipants()[0].displayName) })
            .toggle('Nether', { defaultValue: JSON.parse(ms.world.scoreboard.getObjective('associatedRealmsEssentials:bottomBedrock:nether').getParticipants()[0].displayName) });
        var configureFeaturesCloseRealmForm = new msui.ModalFormData()
            .title('Close Realm (Operators Exempt)')
            .toggle('Enabled', { defaultValue: JSON.parse(ms.world.scoreboard.getObjective('associatedRealmsEssentials:closeRealm:enabled').getParticipants()[0].displayName) })
            .textField('Enter Reason', '(Optional)', { defaultValue: ms.world.scoreboard.getObjective('associatedRealmsEssentials:closeRealm:reason').getParticipants()[0].displayName });
        var configureFeaturesBlockLogsForm = new msui.ModalFormData()
            .title('Block Logs')
            .toggle('Enabled', { defaultValue: JSON.parse(ms.world.scoreboard.getObjective('associatedRealmsEssentials:save:blocklogsenabled').getParticipants()[0].displayName) });
