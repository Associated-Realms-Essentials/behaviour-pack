    ...
    packSettingsForm.show(chatMessage.sender).then(async (packSettingsFormData) => {
            ...
            packSettingsResetPackForm.show(chatMessage.sender).then(async (packSettingsResetPackFormData) => {
                ...
                if (packSettingsResetPackFormData.selection == 1) {
                    ms.world.scoreboard.getObjectives().forEach(dbItem => {
                        if (dbItem.id.includes('associatedRealmsEssentials')) {
        ...
        if (packSettingsFormData.selection == 1) {
            packSettingsRenameAssociationForm.show(chatMessage.sender).then(async (packSettingsRenameAssociationFormData) => {
                ...
                ms.world.scoreboard.removeObjective('associatedRealmsEssentials:save:association');
                ms.world.scoreboard.addObjective('associatedRealmsEssentials:save:association', 'associatedRealmsEssentials:save:association');
                chatMessage.sender.dimension.runCommand(`scoreboard players set "${packSettingsRenameAssociationFormData.formValues[0]}" "associatedRealmsEssentials:save:association" 0`);
            });
            return;
        }
    });
